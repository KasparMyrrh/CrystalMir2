...
namespace Server.MirEnvir
{
    public sealed class ValorSettings
    {
        ...
        public static ValorSettings Load()
        {
            ...
            if (File.Exists(path))
                settings = JsonSerializer.Deserialize<ValorSettings>(File.ReadAllText(path));
            else
            {
            ...
            if (settings == null || string.IsNullOrWhiteSpace(settings.MapFileName)
                ...
                || settings.Rewards == null || settings.Rewards.Any(r => r == null
                    || string.IsNullOrWhiteSpace(r.Item) || r.HonorCost < 1 || r.HonorCost > 200000))
                throw new InvalidDataException("Invalid ValorSettings.json values.");
