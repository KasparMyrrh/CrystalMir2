...
{
    // HUD and scoreboard belong to Valor only; the existing flag HUD has no dependency on this control.
    public sealed class ValorStatusDialog : MirControl
    {
        ...
        public ValorStatusDialog()
        {
            ...
            NotControl = false;
            // The original display has a score plaque, three narrow monument strips,
            // then the two team figures. The countdown is separate at the right edge.
            _hud = new MirControl { Parent = this, Location = new Point(208, 0), Size = new Size(330, 116),
                DrawControlTexture = false };
            Sprite(_hud, 970, 0, 0, 80, 36);
            _blueFill = new ScoreFill { Parent = _hud, Library = Libraries.Prguse2, Index = 971,
                Location = new Point(8, 12), Size = new Size(82, 22), AutoSize = false,
                FillFromRight = false };
            _redFill = new ScoreFill { Parent = _hud, Library = Libraries.Prguse2, Index = 972,
                Location = new Point(98, 11), Size = new Size(82, 22), AutoSize = false,
                FillFromRight = true };
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                _damage[i].Click += (o, e) => ToggleBoard();
                _health[i].Click += (o, e) => ToggleBoard();
            }
            _moon = Sprite(_hud, 980, 29, 34, 20, 20);
            _sun = Sprite(_hud, 983, 113, 34, 20, 20);
            _lightning = Sprite(_hud, 986, 197, 34, 20, 20);
            Sprite(_hud, 990, 9, 57, 34, 35);
            Sprite(_hud, 993, 216, 57, 34, 35);
            _blueCount = Label(_hud, new Point(46, 67), new Size(37, 19), Color.White, 9F);
            _redCount = Label(_hud, new Point(178, 67), new Size(35, 19), Color.White, 9F);
            _clock = Label(GameScene.Scene, new Point(Math.Max(0, Settings.ScreenWidth - 199),
                Math.Max(90, Settings.ScreenHeight - 290)), new Size(170, 45), Color.FromArgb(244, 224, 157), 21F);
            _clock.Visible = false;
            _clock.NotControl = true;
            _clock.DrawFormat = TextFormatFlags.HorizontalCenter | TextFormatFlags.VerticalCenter;
            _hud.Click += (o, e) => ToggleBoard();
            foreach (MirImageControl sprite in new[] { _sun, _moon, _lightning })
                sprite.Click += (o, e) => ToggleBoard();
            foreach (MirLabel label in new[] { _blueScore, _redScore, _blueCount, _redCount })
                label.Click += (o, e) => ToggleBoard();
            _hud.Hint = "Click to view the Valor scoreboard.";

            _board = new MirImageControl { Parent = this, Library = Libraries.Title, Index = 725,
                Size = new Size(684, 463), AutoSize = false, Location = new Point(0, 0), Visible = false };
            var title = Label(_board, new Point(230, 5), new Size(220, 22), Color.White, 11F);
            title.Text = "Battlefield of Valor";
            ...
            string[] headers = { "Name", "Lv.", "Personal", "Kills", "Deaths", "Bonus", "Honor" };
            for (int col = 0; col < 7; col++)
            {
                ...
                {
                    _cells[row, col] = Label(_board, new Point(positions[col] + 2, 120 + row * 20),
                        new Size(widths[col] - 4, 20), Color.White, 8F);
                    _cells[row, col].DrawFormat = col == 0 ? TextFormatFlags.VerticalCenter
        ...
        private static MirLabel Label(MirControl parent, Point location, Size size, Color colour, float fontSize)
            => new MirLabel { Parent = parent, Location = location, Size = size, ForeColour = colour,
                BackColour = Color.Transparent, Font = new Font(Settings.FontName, fontSize),
                OutLine = true, OutLineColour = Color.Black, NotControl = false };
        private static MirImageControl Sprite(MirControl parent, int index, int x, int y, int width, int height)
            => new MirImageControl { Parent = parent, Library = Libraries.Prguse2, Index = index,
                Location = new Point(x, y), Size = new Size(width, height), AutoSize = false };
        private static string Team(byte team) => team == 1 ? "Red" : team == 2 ? "Blue" : "Neutral";
        ...
        public void UpdateStatus(S.ValorStatus status)
        {
            ...
            _status = status;
            if (status.Rows.Count == 0) { Visible = false; _clock.Visible = false; _board.Visible = false; return; }
            Visible = true;
            _hud.Visible = Active;
            ...
            _boardSummary.Text = Active ? $"Red {status.RedScore} / Blue {status.BlueScore}"
                : status.Winner == 0 ? "Result: Draw" : $"Winner: {Team(status.Winner)}";
            if (!Active && wasActive) _board.Visible = true;
            UpdateSize();
            RefreshRows();
        }
        ...
        // MirImageControl.Size only changes hit testing: its DrawControl renders the
        // entire library image. Clip the source sprite to the earned score instead.
        private sealed class ScoreFill : MirImageControl
        {
            ...
            public ScoreFill() { DrawImage = false; }
            ...
            protected internal override void DrawControl()
            {
                ...
                if (width == 0) width = 1;
                // Keep both the source and the destination against the same edge.
                // Blue reveals pixels from its left edge; red reveals from its right.
                int start = FillFromRight ? imageSize.Width - width : 0;
                Point destination = new Point(DisplayLocation.X + start, DisplayLocation.Y);
                Library.Draw(Index, new Rectangle(start, 0, width, imageSize.Height),
                    destination, Color.White, false);
            }
        }
        ...
        private void RefreshRows()
        {
            ...
            for (int row = 0; row < 15; row++)
            {
                ...
                string[] values = entry == null ? new string[7] : new[] { entry.Name, entry.Level.ToString(),
                    entry.Personal.ToString(), entry.Kills.ToString(), entry.Deaths.ToString(), entry.Bonus.ToString(), entry.Honor.ToString() };
                for (int col = 0; col < 7; col++)
                {
                    _cells[row, col].Text = values[col] ?? "";
                    _cells[row, col].ForeColour = entry == null ? Color.White : entry.Team == 1 ? Color.Red : Color.CornflowerBlue;
                }
            }
        }
    }
